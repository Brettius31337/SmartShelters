...
function App() {
  ...
  async function depositShip() {
    const trimmedShipId = shipId.trim();
    if (!selectedShelter || !trimmedShipId) {
  ...
  async function withdrawShip() {
    const trimmedShipId = shipId.trim();
    if (!selectedShelter || !trimmedShipId || !ships.includes(trimmedShipId)) {
  ...
  const shipIdTrimmed = shipId.trim();
  const validDeposit = !!selectedShelter && !!shipIdTrimmed && !loading;
  const validWithdraw = !!selectedShelter && !!shipIdTrimmed && ships.includes(shipIdTrimmed) && !loading;

  return (
    <div style={{padding:'40px',fontFamily:'monospace',color:'#00ff9f',backgroundColor:'#0a0a0a',minHeight:'100vh',display:'flex',flexDirection:'column',alignItems:'center'}}>
              ...
              <div style={{background:'#111',padding:'20px',border:'1px solid #0f0'}}>
                ...
                {ships.length === 0 ? (
                  ...
                  <>
                    ...
                    {ships.map((ship, i) => (
                      <div key={i} style={{margin:'8px 0',padding:'12px',backgroundColor:'#1a1a1a',borderLeft:'3px solid #00ff9f'}}>
                        Ship {i+1}: {ship}
                      </div>
                    ))}
                  </>
      ...
      {!account && (
        <div style={{marginTop:'40px',color:'#555',textAlign:'center'}}>
          Connect your Sui wallet to automatically load your SmartShelters
